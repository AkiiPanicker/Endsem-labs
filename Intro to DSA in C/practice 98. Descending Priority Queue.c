...
// Function to enqueue an element in the descending priority queue
void enqueue() {
    int data;
    printf("Enter data: ");
    scanf("%d", &data);
...
// Function to insert an element at the correct position based on descending priority
void priority(int data) {
    if (queue[rear] > data) { // Change to `>` for descending order
        rear++;
        queue[rear] = data;
    } else {
        for (int i = rear; i >= front; i--) {
            if (queue[i] < data) { // Change to `<` for descending order
